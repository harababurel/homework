...
def rational_multiply(a1, a2, b1, b2):
    ...
    a1,a2,b1,b2 integer numbers, a2<>0 and b2<>0
    return a list with 2 integer numbers, representing a rational number a1/a2 * b1/b2
    Raise ValueError if denominators are zero.
    """
    if a2 == 0 or b2 == 0:
        raise ValueError("0 denominator not allowed")
    c = [a1 * b1, a2 * b2]
...
def calc_multiply(calc, a, b):
    """
    multiply a rational number to the current total
    calc - calculator
    a, b integer numbers, b <> 0
    post: multiply a/b to the current total
    """
    undolist = calc[1]
    calc_total = calc[0]
    #add the current total to the undo list
...
def printMenu():
    ...
    print("   * for multiplying a rational number")
...
def multiplyToCalc(calc):
    """
    Read a rational number and multiply it to the current total
    calc - calculator
    """
    m = input("Give nominator:")
    n = input("Give denominator:")
